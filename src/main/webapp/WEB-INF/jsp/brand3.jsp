...
<body>
	...
	<%-- 참고 하세요 : page : ${page } / count : ${count } / totalPage : ${totalPage } / 
		...
		<c:if test="${page gt 10 }">
			<button onclick="location.href='brand3.do?page=${page - 10 }'">이전</button>
		</c:if>
		<c:if test="${page gt 1 }">
			<button onclick="location.href='brand3.do?page=${page - 1 }'"> ◀ </button>
		</c:if>
		<c:forEach begin="${startPage }" end="${endPage }" var="i">


			<c:if test="${i eq page }">
				<button onclick="location.href='brand3.do?page=${i }'">
		...
		<c:if test="${page lt totalPage - 9 }">
			<button onclick="location.href='brand3.do?page=${page + 10 }'">다음</button>
		</c:if>
	</div>
			<div id="listBottom">
				<select>
