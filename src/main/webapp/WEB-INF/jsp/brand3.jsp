			...
			<c:if test="${brand3 ne null}">
					...
					<c:if test="${page gt 10 }">
						<c:choose>
							<c:when test="${b_cate2 eq null }">
								<div id="pre10" onclick="location.href='brand3.do?page=${page - 10 }'">◀</div>
							</c:when>
							<c:otherwise>
								<div id="pre10" onclick="location.href='brand3.do?page=${page - 10 }&b_cate2=${b_cate2 }'">◀</div>
							</c:otherwise>
						</c:choose>
					</c:if>
					<c:if test="${page gt 1 }">
						<c:choose>
							<c:when test="${b_cate2 eq null }">
								<div id="pre" onclick="location.href='brand3.do?page=${page - 1 }'">◁</div>
							</c:when>
							<c:otherwise>
								<div id="pre" onclick="location.href='brand3.do?page=${page - 1 }&b_cate2=${b_cate2 }'">◁</div>
							</c:otherwise>
						</c:choose>
					</c:if>
				</div>
				<div id="pagenum">
					<c:forEach begin="${startPage }" end="${endPage }" var="i">
						...
						<c:if test="${i ne page }">
							<c:choose>
								<c:when test="${b_cate2 eq null }">
									<div id="ncurpage" onclick="location.href='brand3.do?page=${i }'">${i }</div>
								</c:when>
								<c:otherwise>
									<div id="ncurpage" onclick="location.href='brand3.do?page=${i }&b_cate2=${b_cate2 }'">${i }</div>
								</c:otherwise>
							</c:choose>
						</c:if>
					</c:forEach>
				...
				<div id="pagenex">
					<c:if test="${page lt totalPage }">
						<c:choose>
								<c:when test="${b_cate2 eq null }">
									<div id="nex" onclick="location.href='brand3.do?page=${page + 1 }'">▷</div>
								</c:when>
								<c:otherwise>
									<div id="nex" onclick="location.href='brand3.do?page=${page + 1 }&b_cate2=${b_cate2 }'">▷</div>
								</c:otherwise>
							</c:choose>
					</c:if>
					<c:if test="${page lt totalPage - 9 }">
						<c:choose>
								<c:when test="${b_cate2 eq null }">
									<div id="nex10"	onclick="location.href='brand3.do?page=${page + 10  }'">▶</div>
								</c:when>
								<c:otherwise>
									<div id="nex10"	onclick="location.href='brand3.do?page=${page + 10  }&b_cate2=${b_cate2 }'">▶</div>
								</c:otherwise>
							</c:choose>
					</c:if>
				</div>
